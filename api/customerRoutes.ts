...
const safeDetails = (value: any) => { if (typeof value === "string") return value; try { return JSON.stringify(value ?? ""); } catch { return String(value ?? ""); } };
export async function handleCustomerRoutes(req: any, res: any): Promise<boolean> {
  ...
  try {
    ...
    for (const order of orders as any[]) { const key = String(order.userId || order.email || "").toLowerCase(); if (!key) continue; const current = stats.get(key) || { orders: 0, spent: 0, last: null }; current.orders += 1; current.spent += Number(order.totalAmount || 0); const d = order.createdAt ? new Date(order.createdAt) : null; if (d && (!current.last || d > current.last)) current.last = d; stats.set(key, current); }
    ...
    const rawLogs = Activity ? await Activity.find().sort({ timestamp: -1 }).limit(250).lean() : [];
    const logs = (rawLogs as any[]).map(log => ({ ...log, details: safeDetails(log.details) }));
